...
SAVVYSHERPA MODIFICATIONS
    Taken originally from https://github.com/regeirk/pycwt at commit
    id fa1f0b0e653323e09a9ce342983ab7f0e43849d5 and updated for
...
#originally from pylab import find
def find(condition):
...
def cwt(signal, dt=1., dj=1./12, s0=-1, J=-1, wavelet=Morlet(), result=None):
    ...
    if result == 'dictionary':
        result = dict(
            ...
            #period = 1. / freqs,
            coi = coi,
            signal_ft = signal_ft[1:int(N/2)] / N ** 0.5,
            ftfreqs = ftfreqs[1:int(N/2)] / (2. * pi),
            dt = dt,
            dj = dj,
            s0 = s0,
            J = J,
